WorkerScript.onMessage = function(m) {
    ...
    var reports = []
    var lastdate = 0
    request.open('GET', "https://api.vk.com/api.php?oauth=1&method=wall.get&domain="+m.domain.name+"&count="+m.count+"&access_token=2587470321e1ae26f7xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    request.onreadystatechange = function() {
        if (request.readyState === XMLHttpRequest.DONE)
        {
            if (request.status && request.status === 200)
            {
                if(JSON.parse(request.responseText).response)
                {
                    ...
                    WorkerScript.sendMessage({'error':true})
                }
            }
            else
            {
